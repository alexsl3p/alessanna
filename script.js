  ...
  (function initServiceAndMasterPicks() {
    ...
    function renderList() {
      ...
      if (masterSelectLabel) {
        masterSelectLabel.hidden = !picked.length;
        if (!picked.length && masterSelect) {
          masterSelect.value = "";
    ...
    function wireMenuPickRows() {
      teenused.querySelectorAll(".menu-list li").forEach(function (li) {
        ...
        li.tabIndex = 0;
        /* Ключ ВСЕГДА от русского имени (data-service-name), НЕ от видимого
         * текста span — иначе в ET/EN data-pick-key собирается из перевода,
         * а addPickFromServiceOption/readPickFromLi берут русское имя →
         * pick.key не совпадает с data-pick-key, serviceMastersFromDom
         * возвращает null и форма показывает ВСЕХ мастеров вместо нужных. */
        var pickKeyName = String(li.getAttribute("data-service-name") || nameSpan.textContent || "").trim();
        li.setAttribute("data-pick-key", pickKey(panel, pickKeyName));
    ...
    /* SVG constants for the custom selects. MUST be defined here (before
       buildServiceVisualList() calls makeCsel below) — they were declared as
       var further down, so at this point they'd be undefined and the chevron /
       phone icon would render as the literal text "undefined". */
    ...
    /* ---- Custom select dropdowns for booking form (replaces native white pickers) ----
       PHONE_SVG / CHEVRON_SVG are defined earlier (before buildServiceVisualList). */
