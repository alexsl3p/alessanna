...
export function ReceptionAppointmentDetail({
  ...
  return (
    <div
      ...
      className="fixed z-50 overflow-hidden rounded-xl border border-line/20 bg-panel shadow-[0_18px_50px_rgba(0,0,0,0.5)]"
      role="dialog"
    >
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <p className="text-sm font-semibold leading-tight" style={{ color: colorStyle.color as string }}>
            {appt.client_name}
          </p>
          <button
            onClick={onClose}
      ...
      <div className="space-y-2.5 p-4">
        ...
        <div className="flex items-start gap-3">
          <svg viewBox="0 0 20 20" className="mt-0.5 h-4 w-4 shrink-0 text-muted/60" fill="currentColor">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
          </svg>
          <div>
            <p className="text-sm font-medium capitalize text-fg/90">{dateLabel}</p>
        ...
        {member && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-fg/90">{member.name}</span>
          </div>
        )}
        ...
        {svc && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-fg/90">
              {svc.name_et}
              <span className="ml-1 text-muted/60">· {svc.duration_min} мин</span>
            </span>
          </div>
        )}
        ...
        {appt.client_phone && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-fg/90">{appt.client_phone}</span>
          </div>
        )}
      </div>
    </div>
