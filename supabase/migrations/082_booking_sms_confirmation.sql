-- 082_booking_sms_confirmation.sql
-- SMS-подтверждение записи с сайта.
--
...
-- отправляет через провайдера (Telnyx). Язык SMS = язык страницы, на
...
-- значением по умолчанию). Старый клиент, который его не шлёт, продолжит
-- работать — просто получит язык по умолчанию (et). Поэтому старые
-- перегрузки дропаем и оставляем только новую 9-аргументную.

begin;

drop function if exists public.public_book_chain(text, text, text, timestamptz, jsonb, text, uuid);
drop function if exists public.public_book_chain(text, text, text, timestamptz, jsonb, text, uuid, text);
