-- Public bookings must finish before the salon closes in Europe/Tallinn.
-- Saturdays close at 16:00; other days at 18:00. This also validates
-- multi-service chains and rejects direct RPC calls that bypass the UI.
begin;
...
declare
  ...
  v_lang text := lower(coalesce(nullif(btrim(p_lang), ''), 'et'));
  v_source text;
  v_created_by uuid;
  v_item jsonb;
...
begin
  ...
  if v_lang not in ('ru', 'et', 'en') then
    v_lang := 'et';
  end if;
  ...
  -- Анти-спам: не чаще 1 брони с сайта на один телефон за 5 минут.
  if v_phone is not null and v_source = 'public_site' then
    if exists (
      select 1 from public.appointments a
      where a.client_phone = v_phone
        and a.source = 'public_site'
        and a.created_at > now() - interval '5 minutes'
    ) then
      return jsonb_build_object('ok', false, 'error', 'rate_limited',
        'message', 'Слишком много запросов с этого номера. Попробуйте через 5 минут.');
    end if;
  end if;
  ...
  for v_idx in 0 .. (v_count - 1) loop
    ...
    if v_source = 'public_site' and (
      (v_cursor at time zone 'Europe/Tallinn')::date
        <> (v_end at time zone 'Europe/Tallinn')::date
      or (v_cursor at time zone 'Europe/Tallinn')::time < time '10:00'
      or (v_end at time zone 'Europe/Tallinn')::time >
        case when extract(isodow from v_cursor at time zone 'Europe/Tallinn') = 6
          then time '16:00' else time '18:00' end
    ) then
      return jsonb_build_object('ok', false, 'error', 'outside_salon_hours',
        'message', 'Услуга заканчивается после закрытия салона или начинается до открытия.',
        'hint', jsonb_build_object('index', v_idx, 'start_time', v_cursor, 'end_time', v_end));
    end if;
  ...
  -- SMS-подтверждение: только для записей с сайта и только если есть телефон.
  -- Edge Function `send-booking-sms` заберёт pending-строку и отправит.
  if v_phone is not null and v_source = 'public_site' then
    insert into public.notifications_outbox (appointment_id, kind, payload, status)
    values (
      v_appointment_id,
      'sms',
      jsonb_build_object(
        ...
        'phone', v_phone,
        'client_name', v_name,
        'start_at', (v_items_out->0->>'start_time'),
        'start_local', to_char(
          ((v_items_out->0->>'start_time')::timestamptz) at time zone 'Europe/Tallinn',
          'DD.MM.YYYY HH24:MI'
        ),
        'items', v_items_out,
        'salon_phone', '+372 529 8225'
      ),
      'pending'
    );
  end if;
...
revoke all on function
  public.public_book_chain(text, text, text, timestamptz, jsonb, text, uuid, text, text)
  from public;
...
comment on function
  public.public_book_chain(text, text, text, timestamptz, jsonb, text, uuid, text, text)
is
  'Public booking endpoint for multi-service chain. Persists client profile, note, source, created_by_staff_id and booking language (p_lang). On public_site bookings with a phone, enqueues an SMS confirmation into notifications_outbox (kind=sms).';
