-- 083_booking_rate_limit.sql
-- Анти-спам для публичной брони: не чаще одной брони с одного телефона за
-- 5 минут (защита от накрутки SMS/спама бронями). Redefine public_book_chain
-- (create or replace, сигнатура не меняется) с проверкой перед созданием
-- записи; при срабатывании возвращает ok=false, error='rate_limited'.
